...
The toolkit is intentionally conservative: source-gate and validation are read-only;
write actions never commit, push, merge, or delete branches.
"""
...
def load_config(root: Path) -> dict[str, Any]:
    cfg_path = root / "tools" / "api551" / "api551.config.json"
    if not cfg_path.is_file():
        fail(f"missing toolkit config: {cfg_path}")
    return read_json(cfg_path)


def ensure_file(root: Path, path: str) -> Path:
    p = rel_path(root, path)
    if not p.is_file():
        fail(f"missing required file: {path}")
    return p


def ensure_dir(root: Path, path: str) -> Path:
    p = rel_path(root, path)
    if not p.is_dir():
...
def is_lfs_pointer(path: Path) -> bool:
    ...
    return len(data) <= 512 and data.startswith(LFS_POINTER_PREFIX)
...
def require_real_png_in_package(data: bytes, name: str) -> None:
...
def validate_status_sync(root: Path) -> dict[str, Any]:
    ...
    catalog_stats_field = catalog.get("stats", {})
    ...
    if catalog_stats_field != expected_catalog_stats:
        fail(f"catalog.stats mismatch: {catalog_stats_field} != {expected_catalog_stats}")
...
def docs_sync_check(root: Path, quiet: bool = False) -> dict[str, Any]:
    ...
    accepted_line = f"accepted: {stats['accepted']}/69"
    not_accepted_line = f"not_accepted: {stats['not_accepted']}/69"
    changed_line = f"changed/review: {stats['changed']}"
    required_tool_marker = "tools/api551/api551.ps1"
    required_gate_marker = "source-gate"
    ...
    for doc in docs:
        ...
        text = read_text(doc)
        if required_gate_marker not in text:
    ...
    for marker in [accepted_line, not_accepted_line, changed_line, required_tool_marker]:
        if marker not in bootstrap:
            fail(f"bootstrap status/tooling marker missing: {marker}")

    readme = read_text(root / "README.md")
    quick = read_text(root / "docs" / "API551_PROJECT_QUICK_START_CURRENT.md")
    for text, name in [(readme, "README.md"), (quick, "docs/API551_PROJECT_QUICK_START_CURRENT.md")]:
        if required_tool_marker not in text:
            fail(f"{name} does not mention repo-local toolkit entrypoint")
        if "docs/project/API551_STAGE4_HANDOFF_CURRENT.json" not in text:
            fail(f"{name} does not mention handoff JSON")
...
def source_gate(args: argparse.Namespace) -> None:
    ...
    ensure_file(root, "catalog.json")
    ensure_file(root, "index.html")
    ensure_file(root, ".github/workflows/structure-check.yml")
    ensure_file(root, ".github/workflows/api551-tooling-check.yml")

    stats = docs_sync_check(root, quiet=True)

    git_dir = root / ".git"
    if git_dir.exists() and not args.ci:
        try:
            branch = subprocess.check_output(
                ["git", "branch", "--show-current"], cwd=root, text=True, stderr=subprocess.DEVNULL
            ).strip()
            if branch and branch not in {cfg["working_branch"], "tools/api551-stage4-toolkit-v1"}:
                fail(f"current branch is {branch!r}; expected candidates or an approved PR branch")
...
def figure_check(args: argparse.Namespace) -> None:
    ...
    catalog = read_json(root / "catalog.json")
    if args.all_known:
        figures = [figure_id(f.get("figure_no")) for f in catalog.get("figures", []) if f.get("status") == "accepted"]
    else:
        figures = [figure_id(args.figure)]

    checked = 0
    for fig_id in figures:
        fig = find_catalog_figure(root, fig_id)
        status_value = fig.get("status")
        ...
        if status_value == "accepted":
            if obj.get("approval_status") != "accepted":
                fail(f"Figure {fig_id} object JSON is not accepted")
            if str(obj).find("review_only_not_accepted") >= 0:
        ...
        if status_value == "accepted" and "review_only_not_accepted" in html_text:
            fail(f"Figure {fig_id} object HTML contains review_only_not_accepted")
        for marker in load_config(root).get("service_markers_forbidden_in_out_html", []):
...
def package_check(args: argparse.Namespace) -> None:
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        if fig_id and not any(n.startswith(f"{fig_id}/") for n in names):
        ...
        text_files = [n for n in names if n.lower().endswith((".html", ".json", ".md", ".txt", ".csv"))]
        for name in text_files:
            ...
            if name.endswith(".out.html"):
                for marker in load_config(repo_root()).get("service_markers_forbidden_in_out_html", []):
                    if marker in text:
        ...
        if obj.get("approval_status") == "accepted" and "review_only_not_accepted" in obj_text:
            fail("accepted object JSON contains review_only_not_accepted")

        for name in required:
            if name.lower().endswith(".png"):
                require_real_png_in_package(zf.read(name), name)

        html_names = [f"{fig_id}/figure_{fig_id}.object.html", f"{fig_id}/figure_{fig_id}.out.html"]
        name_set = set(names)
        for html_name in html_names:
            html = zf.read(html_name).decode("utf-8-sig", errors="replace")
            if obj.get("approval_status") == "accepted" and "review_only_not_accepted" in html:
...
def rules_for(args: argparse.Namespace) -> None:
    ...
    report.extend(f"- `{path}`" for path in cfg["current_rule_files"])
    report.append("")
    report.append("## Figure-specific notes")
    if notes:
        report.extend(f"- {note}" for note in notes)
    else:
        report.append("- No dedicated note in tools/api551/rules/figure_rule_map.json; apply universal rules and source-gate.")
    report.append("")
    report.append("## Mandatory workflow")
    report.extend([
...
def pr_check(args: argparse.Namespace) -> None:
    ...
    cmd = [
        ...
        str(args.pr),
        "--repo",
        load_config(root)["repo"],
        "--json",
        "url,state,isDraft,mergeable,headRefName,headRefOid,baseRefName,changedFiles,commits",
    ]
    print(subprocess.check_output(cmd, cwd=root, text=True))
    checks = subprocess.run([gh, "pr", "checks", str(args.pr), "--repo", load_config(root)["repo"]], cwd=root, text=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("source-gate")
    p.add_argument("--ci", action="store_true")
    p.set_defaults(func=source_gate)
    ...
    p = sub.add_parser("docs-sync-check")
    p.add_argument("--ci", action="store_true")
    p.set_defaults(func=lambda args: docs_sync_check(repo_root()))

    p = sub.add_parser("figure-check")
    p.add_argument("-Figure", "--figure", default=None)
    p.add_argument("--all-known", action="store_true")
    p.add_argument("--ci", action="store_true")
    p.set_defaults(func=figure_check)

    p = sub.add_parser("package-check")
    p.add_argument("-PackageZip", "--package-zip", required=True)
    p.add_argument("-Figure", "--figure", default=None)
    p.set_defaults(func=package_check)

    p = sub.add_parser("install-package")
    p.add_argument("-PackageZip", "--package-zip", required=True)
    p.add_argument("-Figure", "--figure", default=None)
    p.set_defaults(func=install_package)

    p = sub.add_parser("rules-for")
    p.add_argument("-Figure", "--figure", required=True)
    p.set_defaults(func=rules_for)

    p = sub.add_parser("open-review")
    p.add_argument("-Figure", "--figure", required=True)
    p.add_argument("--service", action="store_true")
    p.set_defaults(func=open_review)

    p = sub.add_parser("pr-check")
    p.add_argument("-Pr", "--pr", required=True)
    p.set_defaults(func=pr_check)
