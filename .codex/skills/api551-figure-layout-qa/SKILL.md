...
5. drawing geometry is preserved;
6. final PNG, source crop, object metadata, `out.html`, and `object.html` remain consistent;
7. `out.html` is clean export content;
8. `object.html` is review/service content only;
9. `catalog.json` and root `index.html` are updated only when the batch requires it.
